...
function cadastrarJogo(event) {
	...
	const njogo = document.getElementById('njogo').value;
	...
	fetch('http://localhost:8080/jogo', {
		...
		body: JSON.stringify({ njogo, plataform }),
	})
...
function pesquisarJogo() {
	...
	fetch(`http://localhost:8080/jogo/${searchId}`)
		...
		.then(data => {
			result = 1;
			document.getElementById('njogo').value = `${data.name}`;
...
function atualizarJogo() {
	...
	if (result == 1) {
		...
		alert('ID não encontrado na base de dados. Nenhum jogo foi deletado. Favor pesquisar jogo a ser alterado !!!');
	}
}
